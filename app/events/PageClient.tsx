...
/* ─────────────────────────────────────────────────────────────────
   FALLBACK DATA – shown when no events exist in Sanity yet
───────────────────────────────────────────────────────────────── */
const FALLBACK_EVENTS: SanityEvent[] = [
  {
    _id: "fallback-1",
    title: "Night of Liberation Prayer",
    date: "2026-06-06T18:30:00.000Z",
    time: "6:30 PM",
...
function EventCard({ event }: { event: SanityEvent }) {
  ...
  const displayDate = formatSanityDate(event.date);
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className="font-sans font-bold text-gray-900 text-base leading-snug mb-3">
          {event.title}
        </h3>
...
export default function EventsPageClient({ events }: { events: SanityEvent[] }) {
  ...
  useGSAP(
    () => {
      ...
      divTl.to(".ev-divider-star", { rotation: 350, duration: 0.75, ease: "power2.inOut" }, ">");
      divTl.fromTo(
        ".ev-divider-line",
            ...
            {displayEvents.length === 0 ? (
              ...
              <div className="ev-grid grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayEvents.map((event, idx) => (
                  <div
                    key={event._id}
                    className={`ev-card${idx === 3 ? " lg:col-start-1" : ""}`}
                  >
